...
class OnProto(ContinualModel):
    ...
    def observe_task0(self, inputs, labels, not_aug_inputs, epoch=None):
        ...
        # self.buffer.add_reservoir(x=x.detach(), y=y.detach(), logits=None, t=self.task)
    ...
    def observe_other_tasks(self, inputs, labels, not_aug_inputs, epoch=None):
        with autocast():
            # x = x.requires_grad_()
            buffer_batch_size = min(self.minibatch_size, self.buffer_per_class * len(self.seen_so_far))
            ...
            all_images = torch.cat((inputs, images_pair_r), dim=0)

            features = self.net(all_images, returnt='features')
            projections = self.net.projector(features)

            projections_x = projections[:inputs.shape[0]]
            projections_x_r = projections[inputs.shape[0]:]

            projections_x = F.normalize(projections_x)
            projections_x_r = F.normalize(projections_x_r)

            # instance-wise contrastive loss in OCM
            features_x = F.normalize(features[:inputs.shape[0]])
            features_x_r = F.normalize(features[inputs.shape[0]:])
