...
def get_parser() -> ArgumentParser:
    parser = ArgumentParser(description='Continual Prototype Evolution + CSSL')
    ...
    def observe(self, inputs, labels, not_aug_inputs, epoch=None):
        ...
        for i in range(self.args.num_batches):
            ...
            if not self.buffer.is_empty():
                ...
                all_not_aug_inputs = torch.cat([not_aug_inputs, buf_not_aug_inputs], dim=0)
            ...
            all_inputs_b = self.aug_batch(all_not_aug_inputs, device=inputs.device)
            loss_inv = self.selffunc(self.net, all_inputs, all_inputs_b)
            loss += self.args.inv_alpha * loss_inv
            ...
            if update:
                ...
                # And Update
                self.update_protos()
                self.buffer.add_data(examples=not_aug_inputs.data,
                                labels=labels)
