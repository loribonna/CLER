...
class XDerCssl(ContinualModel):
    ...
    def observe(self, inputs, labels, not_aug_inputs, epoch=None):
        ...
        if not self.buffer.is_empty():
            ...
            all_not_aug_inputs = torch.cat([not_aug_inputs, buf_not_aug_inputs_ce, buf_not_aug_inputs_mse])
            all_inputs = torch.cat([inputs, buf_inputs2, buf_inputs1])
        ...
        if self.args.inv_alpha > 0:  # computable on stream and buffer
            loss_inv = self.selffunc(self.net, all_inputs, self.aug_batch(all_not_aug_inputs, device=self.device))
