...
class Buffer(nn.Module):
    def __init__(self, args, device, input_size=None):
        super().__init__()
        self.args = args
        self.device = device
        self.k = 0.03
    ...
    def display(self, gen=None, epoch=-1):
        ...
        if 'cifar' in self.args.dataset:
            ...
            shp = (-1, 3, 64, 64)
        elif 'miniimg' in self.args.dataset:
            shp = (-1, 3, 84, 84)
        else:
            shp = (-1, 1, 28, 28)
    ...
    def add_reservoir(self, x, y, logits, t):
        ...
        self.bx[idx_buffer] = x[idx_new_data].to(self.device)
        self.by[idx_buffer] = y[idx_new_data].to(self.device)
